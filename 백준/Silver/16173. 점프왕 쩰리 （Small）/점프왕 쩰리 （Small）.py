...
def bfs(x, y):
    ...
    while queue:
        ...
        for i in range(2):
            # nx = 이동할 x 좌표
            # ny = 이동할 y 좌표
            nx = x + dx[i] * move  # x좌표 이동
            ny = y + dy[i] * move  # y좌표 이동
            
            # 이동한 범위가 좌표를 넘어갈 때 continue
            if nx < 0 or nx >= n or ny < 0 or ny >= n:
                continue
            
            # 방문하지 않았을 때 큐에 추가하고 방문처리를 하는 로직
            if not visited[nx][ny]:
                visited[nx][ny] = True
                # nx, ny를 리스트로 queue에 추가
                queue.append([nx, ny])

    # 큐가 모두 비었는데 목적지에 도달하지 못하면 False를 반환
    return False
