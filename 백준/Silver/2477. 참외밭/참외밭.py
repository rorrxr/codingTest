...
반시계방향으로 남쪽으로 30m, 동쪽으로 60m, 
남쪽으로 20m, 동쪽으로 100m, 
북쪽으로 50m, 서쪽으로 160m 이동하면 다시 출발점으로 되돌아가게 된다.
...
반시계방향으로 둘레를 돌면서 지나는 변의 방향과 길이 (1 이상 500 이하의 정수) 가 둘째 줄부터 일곱 번째 줄까지 한 줄에 하나씩 순서대로 주어진다. 
...
# 입력 처리
K = int(input())  # 1m² 당 참외 개수
directions_lengths = [tuple(map(int, input().split())) for _ in range(6)]

# 변의 방향과 길이 분리
directions = [d for d, _ in directions_lengths]
lengths = [l for _, l in directions_lengths]
...
for i in range(6):
    if directions[i] == 1 or directions[i] == 2:  # 동서 방향
        if lengths[i] > max_width:
            max_width = lengths[i]
            max_width_idx = i
    elif directions[i] == 3 or directions[i] == 4:  # 남북 방향
        if lengths[i] > max_height:
            max_height = lengths[i]
            max_height_idx = i

# 작은 직사각형의 넓이를 계산
small_width = lengths[(max_height_idx + 3) % 6]
small_height = lengths[(max_width_idx + 3) % 6]

# 면적 계산
large_area = max_width * max_height
small_area = small_width * small_height
area = large_area - small_area
